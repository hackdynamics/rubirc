...
#include "text.h"
#define PLUGIN_C
typedef struct session rubirc_context;
...
struct _rubirc_hook
{
	rubirc_plugin *pl;	/* the plugin to which it belongs */
	char *name;			/* "xdcc" */
...
typedef int (rubirc_cmd_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (rubirc_serv_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (rubirc_print_cb) (char *word[], void *user_data);
typedef int (rubirc_serv_attrs_cb) (char *word[], char *word_eol[], rubirc_event_attrs *attrs, void *user_data);
typedef int (rubirc_print_attrs_cb) (char *word[], rubirc_event_attrs *attrs, void *user_data);
typedef int (rubirc_fd_cb) (int fd, int flags, void *user_data);
typedef int (rubirc_timer_cb) (void *user_data);
typedef int (rubirc_init_func) (rubirc_plugin *, char **, char **, char **, char *);
typedef int (rubirc_deinit_func) (rubirc_plugin *);
...
static int
plugin_free (rubirc_plugin *pl, int do_deinit, int allow_refuse)
{
	GSList *list, *next;
	rubirc_hook *hook;
	rubirc_deinit_func *deinit_func;

	/* fake plugin added by rubirc_plugingui_add() */
	if (pl->fake)
		goto xit;
...
static rubirc_plugin *
plugin_list_add (rubirc_context *ctx, char *filename, const char *name,
					  const char *desc, const char *version, void *handle,
					  void *deinit_func, int fake, int free_strings)
{
	rubirc_plugin *pl;

	pl = g_new (rubirc_plugin, 1);
	pl->handle = handle;
...
static int
rubirc_read_fd (rubirc_plugin *ph, GIOChannel *source, char *buf, int *len)
{
	GError *error = NULL;
				...
				void *deinit_func, char *arg, int fake)
{
	...
	if (!fake)
	{
		...
		pl->rubirc_hook_command = rubirc_hook_command;
		pl->rubirc_hook_server = rubirc_hook_server;
		pl->rubirc_hook_print = rubirc_hook_print;
		pl->rubirc_hook_timer = rubirc_hook_timer;
		pl->rubirc_hook_fd = rubirc_hook_fd;
		pl->rubirc_unhook = rubirc_unhook;
		pl->rubirc_print = rubirc_print;
		pl->rubirc_printf = rubirc_printf;
		pl->rubirc_command = rubirc_command;
		pl->rubirc_commandf = rubirc_commandf;
		pl->rubirc_nickcmp = rubirc_nickcmp;
		pl->rubirc_set_context = rubirc_set_context;
		pl->rubirc_find_context = rubirc_find_context;
		pl->rubirc_get_context = rubirc_get_context;
		pl->rubirc_get_info = rubirc_get_info;
		pl->rubirc_get_prefs = rubirc_get_prefs;
		pl->rubirc_list_get = rubirc_list_get;
		pl->rubirc_list_free = rubirc_list_free;
		pl->rubirc_list_fields = rubirc_list_fields;
		pl->rubirc_list_str = rubirc_list_str;
		pl->rubirc_list_next = rubirc_list_next;
		pl->rubirc_list_int = rubirc_list_int;
		pl->rubirc_plugingui_add = rubirc_plugingui_add;
		pl->rubirc_plugingui_remove = rubirc_plugingui_remove;
		pl->rubirc_emit_print = rubirc_emit_print;
#ifdef WIN32
		pl->rubirc_read_fd = (void *) rubirc_read_fd;
#else
		pl->rubirc_read_fd = rubirc_dummy;
#endif
		pl->rubirc_list_time = rubirc_list_time;
		pl->rubirc_gettext = rubirc_gettext;
		pl->rubirc_send_modes = rubirc_send_modes;
		pl->rubirc_strip = rubirc_strip;
		pl->rubirc_free = rubirc_free;
		pl->rubirc_pluginpref_set_str = rubirc_pluginpref_set_str;
		pl->rubirc_pluginpref_get_str = rubirc_pluginpref_get_str;
		pl->rubirc_pluginpref_set_int = rubirc_pluginpref_set_int;
		pl->rubirc_pluginpref_get_int = rubirc_pluginpref_get_int;
		pl->rubirc_pluginpref_delete = rubirc_pluginpref_delete;
		pl->rubirc_pluginpref_list = rubirc_pluginpref_list;
		pl->rubirc_hook_server_attrs = rubirc_hook_server_attrs;
		pl->rubirc_hook_print_attrs = rubirc_hook_print_attrs;
		pl->rubirc_emit_print_attrs = rubirc_emit_print_attrs;
		pl->rubirc_event_attrs_create = rubirc_event_attrs_create;
		pl->rubirc_event_attrs_free = rubirc_event_attrs_free;

		/* run rubirc_plugin_init, if it returns 0, close the plugin */
		if (((rubirc_init_func *)init_func) (pl, &pl->name, &pl->desc, &pl->version, arg) == 0)
		{
...
plugin_load (session *sess, char *filename, char *arg)
{
	...
	rubirc_init_func *init_func;
	rubirc_deinit_func *deinit_func;

	if (handle == NULL)
		return (char *)g_module_error ();

	/* find the init routine rubirc_plugin_init */
	if (!g_module_symbol (handle, "rubirc_plugin_init", (gpointer *)&init_func))
	{
		g_module_close (handle);
		return _("No rubirc_plugin_init symbol; is this really a RUBIRC plugin?");
	}

	/* find the plugin's deinit routine, if any */
	if (!g_module_symbol (handle, "rubirc_plugin_deinit", (gpointer *)&deinit_func))
		deinit_func = NULL;
...
plugin_hook_run (session *sess, char *name, char *word[], char *word_eol[],
				 rubirc_event_attrs *attrs, int type)
{
	GSList *list, *next;
	rubirc_hook *hook;
	int ret, eat = 0;
	...
	while (1)
	{
		...
		{
		case HOOK_COMMAND:
			ret = ((rubirc_cmd_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_PRINT_ATTRS:
			ret = ((rubirc_print_attrs_cb *)hook->callback) (word, attrs, hook->userdata);
			break;
		case HOOK_SERVER:
			ret = ((rubirc_serv_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_SERVER_ATTRS:
			ret = ((rubirc_serv_attrs_cb *)hook->callback) (word, word_eol, attrs, hook->userdata);
			break;
		default: /*case HOOK_PRINT:*/
			ret = ((rubirc_print_cb *)hook->callback) (word, hook->userdata);
			break;
		}

		if ((ret & RUBIRC_EAT_RUBIRC) && (ret & RUBIRC_EAT_PLUGIN))
		{
			eat = 1;
			goto xit;
		}
		if (ret & RUBIRC_EAT_PLUGIN)
			goto xit;	/* stop running plugins */
		if (ret & RUBIRC_EAT_RUBIRC)
...
rubirc_event_attrs *
rubirc_event_attrs_create (rubirc_plugin *ph)
{
	return g_new0 (rubirc_event_attrs, 1);
}

void
rubirc_event_attrs_free (rubirc_plugin *ph, rubirc_event_attrs *attrs)
{
	g_free (attrs);
}
...
static int
plugin_timeout_cb (rubirc_hook *hook)
{
	int ret;
	...
	ret = ((rubirc_timer_cb *)hook->callback) (hook->userdata);
...
static void
plugin_insert_hook (rubirc_hook *new_hook)
{
	GSList *list;
	rubirc_hook *hook;
	int new_hook_type;
...
static gboolean
plugin_fd_cb (GIOChannel *source, GIOCondition condition, rubirc_hook *hook)
{
	int flags = 0, ret;
	typedef int (rubirc_fd_cb2) (int fd, int flags, void *user_data, GIOChannel *);

	if (condition & G_IO_IN)
		flags |= RUBIRC_FD_READ;
	if (condition & G_IO_OUT)
		flags |= RUBIRC_FD_WRITE;
	if (condition & G_IO_PRI)
		flags |= RUBIRC_FD_EXCEPTION;

	ret = ((rubirc_fd_cb2 *)hook->callback) (hook->pri, flags, hook->userdata, source);
...
static rubirc_hook *
plugin_add_hook (rubirc_plugin *pl, int type, int pri, const char *name,
					  const  char *help_text, void *callb, int timeout, void *userdata)
{
	rubirc_hook *hook;

	hook = g_new0 (rubirc_hook, 1);
	hook->type = type;
	hook->pri = pri;
...
void *
rubirc_unhook (rubirc_plugin *ph, rubirc_hook *hook)
{
	/* perl.c trips this */
	if (!g_slist_find (hook_list, hook) || hook->type == HOOK_DELETED)
		return NULL;
...
rubirc_hook *
rubirc_hook_command (rubirc_plugin *ph, const char *name, int pri,
						  rubirc_cmd_cb *callb, const char *help_text, void *userdata)
{
...
rubirc_hook *
rubirc_hook_server (rubirc_plugin *ph, const char *name, int pri,
						 rubirc_serv_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_SERVER, pri, name, 0, callb, 0, userdata);
}

rubirc_hook *
rubirc_hook_server_attrs (rubirc_plugin *ph, const char *name, int pri,
						   rubirc_serv_attrs_cb *callb, void *userdata)
{
...
rubirc_hook *
rubirc_hook_print (rubirc_plugin *ph, const char *name, int pri,
						rubirc_print_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_PRINT, pri, name, 0, callb, 0, userdata);
}

rubirc_hook *
rubirc_hook_print_attrs (rubirc_plugin *ph, const char *name, int pri,
						  rubirc_print_attrs_cb *callb, void *userdata)
{
...
rubirc_hook *
rubirc_hook_timer (rubirc_plugin *ph, int timeout, rubirc_timer_cb *callb,
					   void *userdata)
{
...
rubirc_hook *
rubirc_hook_fd (rubirc_plugin *ph, int fd, int flags,
					rubirc_fd_cb *callb, void *userdata)
{
	rubirc_hook *hook;
...
void
rubirc_print (rubirc_plugin *ph, const char *text)
{
	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\trubirc_print called without a valid context.\n", ph->name));
		return;
	}
...
void
rubirc_printf (rubirc_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
...
void
rubirc_command (rubirc_plugin *ph, const char *command)
{
	char *command_utf8;

	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\trubirc_command called without a valid context.\n", ph->name));
		return;
	}
...
void
rubirc_commandf (rubirc_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	rubirc_command (ph, buf);
	g_free (buf);
}

int
rubirc_nickcmp (rubirc_plugin *ph, const char *s1, const char *s2)
{
...
int
rubirc_set_context (rubirc_plugin *ph, rubirc_context *context)
{
...
rubirc_context *
rubirc_find_context (rubirc_plugin *ph, const char *servname, const char *channel)
{
	return plugin_find_context (servname, channel, ph->context->server);
}

const char *
rubirc_get_info (rubirc_plugin *ph, const char *id)
{
	session *sess;
	guint32 hash;
	...
	if (!is_session (sess))
	{
		DEBUG(PrintTextf(0, "%s\trubirc_get_info called without a valid context.\n", ph->name));
		return NULL;
	}
...
int
rubirc_get_prefs (rubirc_plugin *ph, const char *name, const char **string, int *integer)
{
	int i = 0;
...
rubirc_list *
rubirc_list_get (rubirc_plugin *ph, const char *name)
{
	rubirc_list *list;

	list = g_new0 (rubirc_list, 1);
...
void
rubirc_list_free (rubirc_plugin *ph, rubirc_list *xlist)
{
...
int
rubirc_list_next (rubirc_plugin *ph, rubirc_list *xlist)
{
	if (xlist->next == NULL)
		return 0;
...
const char * const *
rubirc_list_fields (rubirc_plugin *ph, const char *name)
{
...
time_t
rubirc_list_time (rubirc_plugin *ph, rubirc_list *xlist, const char *name)
{
...
const char *
rubirc_list_str (rubirc_plugin *ph, rubirc_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
...
int
rubirc_list_int (rubirc_plugin *ph, rubirc_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
	gpointer data = ph->context;
...
void *
rubirc_plugingui_add (rubirc_plugin *ph, const char *filename,
...
void
rubirc_plugingui_remove (rubirc_plugin *ph, void *handle)
{
#ifdef USE_PLUGIN
...
int
rubirc_emit_print (rubirc_plugin *ph, const char *event_name, ...)
{
	va_list args;
...
char *
rubirc_gettext (rubirc_plugin *ph, const char *msgid)
{
	/* so that plugins can use RUBIRC's internal gettext strings. */
	/* e.g. The EXEC plugin uses this on Windows. */
	return _(msgid);
}

void
rubirc_send_modes (rubirc_plugin *ph, const char **targets, int ntargets, int modes_per_line, char sign, char mode)
{
...
char *
rubirc_strip (rubirc_plugin *ph, const char *str, int len, int flags)
{
	return strip_color ((char *)str, len, flags);
}

void
rubirc_free (rubirc_plugin *ph, void *ptr)
{
	g_free (ptr);
}

static int
rubirc_pluginpref_set_str_real (rubirc_plugin *pl, const char *var, const char *value, int mode) /* mode: 0 = delete, 1 = save */
{
	FILE *fpIn;
	int fhOut;
	int prevSetting;
	...
	fhOut = rubirc_open_file (confname_tmp, O_TRUNC | O_WRONLY | O_CREAT, 0600, XOF_DOMODE);
	fpIn = rubirc_fopen_file (confname, "r", 0);
...
int
rubirc_pluginpref_set_str (rubirc_plugin *pl, const char *var, const char *value)
{
	return rubirc_pluginpref_set_str_real (pl, var, value, 1);
}

static int
rubirc_pluginpref_get_str_real (rubirc_plugin *pl, const char *var, char *dest, int dest_len)
{
...
int
rubirc_pluginpref_get_str (rubirc_plugin *pl, const char *var, char *dest)
{
	/* All users of this must ensure dest is >= 512... */
	return rubirc_pluginpref_get_str_real (pl, var, dest, 512);
}

int
rubirc_pluginpref_set_int (rubirc_plugin *pl, const char *var, int value)
{
	char buffer[12];

	g_snprintf (buffer, sizeof (buffer), "%d", value);
	return rubirc_pluginpref_set_str_real (pl, var, buffer, 1);
}

int
rubirc_pluginpref_get_int (rubirc_plugin *pl, const char *var)
{
	char buffer[12];

	if (rubirc_pluginpref_get_str_real (pl, var, buffer, sizeof(buffer)))
	{
		int ret = atoi (buffer);
...
int
rubirc_pluginpref_delete (rubirc_plugin *pl, const char *var)
{
	return rubirc_pluginpref_set_str_real (pl, var, 0, 0);
}

int
rubirc_pluginpref_list (rubirc_plugin *pl, char* dest)
{
	FILE *fpIn;
	char confname[64];
