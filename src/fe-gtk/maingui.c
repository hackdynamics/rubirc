...
/*	printf("enter mg_topdestroy. sess %p was destroyed\n", sess);*/
	session_free (sess);	/* tell rubirc.c about it */
}
...
mg_open_quit_dialog (gboolean minimize_button)
{
	...
	gtk_window_set_title (GTK_WINDOW (dialog), _("Quit RUBIRC?"));
...
mg_create_chanmodebuttons (session_gui *gui, GtkWidget *box)
{
	...
	gtk_widget_set_name (gui->key_entry, "rubirc-inputbox");
	...
	gtk_widget_set_name (gui->limit_entry, "rubirc-inputbox");
...
mg_create_topicbar (session *sess, GtkWidget *box)
{
	...
	gtk_widget_set_name (topic, "rubirc-inputbox");
...
mg_create_textarea (session *sess, GtkWidget *box)
{
	...
	{
		{"RUBIRC_CHANVIEW", GTK_TARGET_SAME_APP, 75 },
		{"RUBIRC_USERLIST", GTK_TARGET_SAME_APP, 75 }
	};
...
mg_create_entry (session *sess, GtkWidget *box)
{
	...
	gtk_widget_set_name (entry, "rubirc-inputbox");
...
mg_create_topwindow (session *sess)
{
	...
	if (sess->type == SESS_DIALOG)
		win = gtkutil_window_new ("RUBIRC", NULL,
...
mg_create_tabwindow (session *sess)
{
	...
	win = gtkutil_window_new ("RUBIRC", NULL, prefs.hex_gui_win_width,
									  prefs.hex_gui_win_height, 0);
	sess->gui->window = win;
...
mg_is_gui_target (GdkDragContext *context)
{
	...
	if (target_name)
	{
		/* if it's not RUBIRC_CHANVIEW or RUBIRC_USERLIST */
		/* we should ignore it. */
		if (target_name[0] != 'H')
		{
			g_free (target_name);
