...
static struct rubircprefs setup_prefs;
...
setup_create_color_button (GtkWidget *table, int num, int row, int col)
{
	...
	g_object_set_data (G_OBJECT (but), "rubirc-color", (gpointer)1);
...
setup_snd_filereq_cb (GtkWidget *entry, char *file)
{
	if (file)
	{
		if (file[0])
		{
			 ...
			 */
			if (!strcmp (g_path_get_dirname (file), g_build_filename (get_xdir (), RUBIRC_SOUND_DIR, NULL)))
			{
...
{
	char *sounds_dir = g_build_filename (get_xdir (), RUBIRC_SOUND_DIR, NULL);
	char *filter = NULL;
	int filter_type;
...
static void
setup_apply (struct rubircprefs *pr)
{
#ifdef WIN32
