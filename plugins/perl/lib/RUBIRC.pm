$SIG{__WARN__} = sub {
	...
	if( $message =~ /^(?:Gtk|GLib|Gdk)(?:-\w+)?-$message_levels/i ) {
		...
		if( defined &RUBIRC::Internal::print ) {
			RUBIRC::print( $message );
		} else {
			warn $message;
		}
	}
...
sub get_context;
sub RUBIRC::Internal::context_info;
sub RUBIRC::Internal::print;
...
our %EXPORT_TAGS = (
	constants => [
		...
		qw(EAT_NONE EAT_RUBIRC EAT_XCHAT EAT_PLUGIN EAT_ALL), # callback return values
...
sub register {
	my ($package, $calling_package) = RUBIRC::Embed::find_pkg();
	my $pkg_info = RUBIRC::Embed::pkg_info( $package );
	my $filename = $pkg_info->{filename};
	my ($name, $version, $description, $callback) = @_;
	
	if( defined $pkg_info->{gui_entry} ) {
		RUBIRC::print( "RUBIRC::register called more than once in "
			. $pkg_info->{filename} );
		return ();
	}
	...
	if( $callback ) {
		$callback = RUBIRC::Embed::fix_callback(
	...
	$pkg_info->{gui_entry} =
		RUBIRC::Internal::register( $name, $version, $description, $filename );
...
sub hook_server {
	...
	my ($priority, $data) = ( RUBIRC::PRI_NORM, undef );
	...
	my $pkg_info = RUBIRC::Embed::pkg_info( $package );
	my $hook = RUBIRC::Internal::hook_server(
...
sub hook_command {
	...
	my ($priority, $help_text, $data) = ( RUBIRC::PRI_NORM, undef, undef );
	_process_hook_options(
		$options,
	...
	my $pkg_info = RUBIRC::Embed::pkg_info( $package );
	my $hook = RUBIRC::Internal::hook_command(
...
sub hook_print {
	...
	my ($priority, $run_after, $filter, $data) = ( RUBIRC::PRI_NORM, 0, 0, undef );
	_process_hook_options(
		$options,
	...
	if( $run_after and $filter ) {
		Carp::carp( "RUBIRC::hook_print's run_after_event and filter options are mutually exclusive, you can only use of them at a time per hook" );
		return;
	}
	...
	my $pkg_info = RUBIRC::Embed::pkg_info( $package );
	my $hook = RUBIRC::Internal::hook_print(
...
sub hook_timer {
	...
	my $pkg_info = RUBIRC::Embed::pkg_info( $package );
	my $hook = RUBIRC::Internal::hook_timer( $timeout, $callback, $data, $package );
	push @{$pkg_info->{hooks}}, $hook if defined $hook;
	return $hook;
}
...
sub hook_fd {
	...
	my ($flags, $data) = (RUBIRC::FD_READ, undef);
	_process_hook_options(
		$options,
		[qw(flags data)],
		[\($flags, $data)],
	...
	my $pkg_info = RUBIRC::Embed::pkg_info( $package );
	my $hook = RUBIRC::Internal::hook_fd(
		$fileno, $cb, $flags, {
...
sub unhook {
	...
	if( defined( $hook )
		...
		return RUBIRC::Internal::unhook( $hook );
	}
	return ();
}
...
sub _do_for_each {
	...
	my $num_done = 0;
	my $old_ctx = RUBIRC::get_context();
	for my $server ( @$servers ) {
		for my $channel ( @$channels ) {
			if( RUBIRC::set_context( $channel, $server ) ) {
				$cb->();
				$num_done++
			}
		}
	}
	RUBIRC::set_context( $old_ctx );
	return $num_done;
}
...
sub print {
	...
	return _do_for_each(
		sub { RUBIRC::Internal::print( $text ); },
		@_
	);
}
...
sub printf {
	my $format = shift;
	RUBIRC::print( sprintf( $format, @_ ) );
}

# make RUBIRC::prnt() and RUBIRC::prntf() as aliases for RUBIRC::print() and 
# RUBIRC::printf(), mainly useful when these functions are exported
sub prnt {
	goto &RUBIRC::print;
}
...
sub command {
	...
	return _do_for_each(
		sub { RUBIRC::Internal::command( $_ ) foreach @commands },
		@_
	);
}
...
sub commandf {
	my $format = shift;
	RUBIRC::command( sprintf( $format, @_ ) );
}
...
sub plugin_pref_set {
	...
	return RUBIRC::Internal::plugin_pref_set($setting, $value);
}

sub plugin_pref_get {
	my $setting = shift // return 0;

	return RUBIRC::Internal::plugin_pref_get($setting);
}

sub plugin_pref_delete {
	my $setting = shift // return 0;

	return RUBIRC::Internal::plugin_pref_delete($setting);
}

sub plugin_pref_list {
	my %list = RUBIRC::Internal::plugin_pref_list();
...
sub set_context {
	...
	if( @_ == 2 ) {
		my ($channel, $server) = @_;
		$context = RUBIRC::find_context( $channel, $server );
	} elsif( @_ == 1 ) {
		if( defined $_[0] && $_[0] =~ /^\d+$/ ) {
			$context = $_[0];
		} else {
			$context = RUBIRC::find_context( $_[0] );
		}
	} elsif( @_ == 0 ) {
		$context = RUBIRC::find_context();
	}
	return $context ? RUBIRC::Internal::set_context( $context ) : 0;
}
...
sub get_info {
	...
	if( defined( $id ) ) {
		if( grep { $id eq $_ } qw(state_cursor id) ) {
			$info = RUBIRC::get_prefs( $id );
		} else {
			$info = RUBIRC::Internal::get_info( $id );
		}
	}
	return $info;
}
...
sub user_info {
	my $nick = RUBIRC::strip_code(shift @_ || RUBIRC::get_info( "nick" ));
	my $user;
	for (RUBIRC::get_list( "users" ) ) {
		if ( RUBIRC::nickcmp( $_->{nick}, $nick ) == 0 ) {
			$user = $_;
			last;
		}
	}
...
sub context_info {
	my $ctx = shift @_ || RUBIRC::get_context;
	my $old_ctx = RUBIRC::get_context;
	my @fields = (
	...
	if( RUBIRC::set_context( $ctx ) ) {
		my %info;
		for my $field ( @fields ) {
			$info{$field} = RUBIRC::get_info( $field );
		}
		
		my $ctx_info = RUBIRC::Internal::context_info;
...
sub get_list {
	...
	if( $_[0] eq 'networks' ) {
		return RUBIRC::List::Network->get();
	} else {
		return RUBIRC::Internal::get_list( $_[0] );
	}
}
