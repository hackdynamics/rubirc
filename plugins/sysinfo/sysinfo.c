/*
 * SysInfo - sysinfo plugin for RUBIRC
...
get_client (void)
{
	return g_strdup_printf ("RUBIRC %s", rubirc_get_info(ph, "version"));
}
...
print_summary (gboolean announce)
{
	...
	rubirc_commandf (ph, "%s %s", announce ? "SAY" : "ECHO", output);
...
print_info (char *info, gboolean announce)
{
	...
	for (i = 0; hwinfos[i].name != NULL; i++)
	{
		if (!g_ascii_strcasecmp (info, hwinfos[i].name))
		{
			...
			if (str)
			{
				rubirc_commandf (ph, "%s \002%s\002: %s", announce ? "SAY" : "ECHO",
									hwinfos[i].title, str);
				g_free (str);
			}
			else
				rubirc_print (ph, _("Sysinfo: Failed to get info. Either not supported or error."));
			return;
		}
	}

	rubirc_print (ph, _("Sysinfo: No info by that name\n"));
}

static gboolean
sysinfo_get_bool_pref (const char *pref, gboolean def)
{
	int value = rubirc_pluginpref_get_int (ph, pref);
...
sysinfo_set_pref_real (const char *pref, char *value, gboolean def)
{
	if (value && value[0])
	{
		...
		rubirc_pluginpref_set_int (ph, pref, i != 0);
		rubirc_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref, i != 0);
	}
	else
	{
		rubirc_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref,
...
sysinfo_set_pref (char *key, char *value)
{
	if (!key || !key[0])
	{
		rubirc_print (ph, _("Sysinfo: Valid settings are: announce and hide_* for each piece of information. e.g. hide_os. Without a value it will show current (or default) setting.\n"));
		return;
	}
	...
	rubirc_print (ph, _("Sysinfo: Invalid variable name\n"));
}
...
sysinfo_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	channel_type = rubirc_list_int (ph, NULL, "type");
...
rubirc_plugin_init (rubirc_plugin *plugin_handle, char **plugin_name, char **plugin_desc, char **plugin_version, char *arg)
{
	...
	*plugin_version = version;
	...
	rubirc_hook_command (ph, "SYSINFO", RUBIRC_PRI_NORM, sysinfo_cb, sysinfo_help, NULL);

	rubirc_command (ph, "MENU ADD \"Window/Send System Info\" \"SYSINFO\"");
	rubirc_printf (ph, _("%s plugin loaded\n"), name);
	return 1;
}
...
{
	rubirc_command (ph, "MENU DEL \"Window/Display System Info\"");
	rubirc_printf (ph, _("%s plugin unloaded\n"), name);
	return 1;
}
