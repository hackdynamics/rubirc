...
#ifdef USE_DBUS
 {"existing",	'e', 0, G_OPTION_ARG_NONE,	&arg_existing, N_("Open URL or execute command in an existing RUBIRC"), NULL},
#endif
...
#ifndef WIN32
	pixmaps_init ();
	gtk_window_set_icon (GTK_WINDOW (dialog), pix_rubirc);
#endif
...
const char cursor_color_rc[] =
	...
	"}"
	"widget \"*.rubirc-inputbox\" style : application \"xc-ib-st\"";

static const char adwaita_workaround_rc[] =
	"style \"rubirc-input-workaround\""
	"{"
		"engine \"pixmap\" {"
			"image {"
				"function = FLAT_BOX\n"
		...
		"}"
	"}"
	"widget \"*.rubirc-inputbox\" style \"rubirc-input-workaround\"";
...
#ifdef HAVE_GTK_MAC
	gtkosx_application_set_dock_icon_pixbuf (osx_app, pix_rubirc);
#endif
		       ...
		       gpointer	      unused_data)
{
	...
	/* if (getenv ("RUBIRC_WARNING_IGNORE")) this gets ignored sometimes, so simply just disable all warnings */
		return;
	...
	PrintTextf (sess, "%s\t%s\n", log_domain, message);
	if (getenv ("RUBIRC_WARNING_ABORT"))
		abort ();
}
