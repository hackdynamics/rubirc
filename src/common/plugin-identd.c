...
{
	g_return_val_if_fail (responses != NULL, RUBIRC_EAT_ALL);
	...
	if (word[2] && *word[2] && word[3] && *word[3])
	{
		...
		if (port && port <= G_MAXUINT16)
		{
			...
			rubirc_hook_timer (ph, 30000, identd_cleanup_response_cb, GINT_TO_POINTER (port));
		}
	}
	else
	{
		rubirc_command (ph, "HELP IDENTD");
	}
		...
		else
		{
			...
			else
			{
				...
				if ((sok_addr = g_socket_connection_get_remote_address (info->conn, NULL)))
				{
					...
					rubirc_printf (ph, _("*\tServicing ident request from %s as %s"), addr, info->username);
...
identd_start_server (void)
{
	...
	if (rubirc_get_prefs (ph, "identd_server", NULL, &enabled) == 3)
	{
		if (!enabled)
			return;
	}
	if (rubirc_get_prefs (ph, "identd_port", NULL, &port) == 2 && (port <= 0 || port > G_MAXUINT16))
	{
		port = 113;
	}
	...
	if (error)
	{
		rubirc_printf (ph, _("*\tError starting identd server: %s"), error->message);

		g_error_free (error);
		g_clear_object (&service);
		return;
	}
	/*rubirc_printf (ph, "*\tIdentd listening on port: %d", port); */
...
int
identd_plugin_init (rubirc_plugin *plugin_handle, char **plugin_name,
					char **plugin_desc, char **plugin_version, char *arg)
{
	...
	*plugin_version = "";
	...
	rubirc_hook_command (ph, "IDENTD", RUBIRC_PRI_NORM, identd_command_cb,
