...
winamp(char *word[], char *word_eol[], void *userdata)
{
	...
	if (hwndWinamp)
	{
		if (!stricmp("PAUSE", word[2]))
		{
			if (SendMessage(hwndWinamp,WM_USER, 0, 104))
			{
				...
				if (SendMessage(hwndWinamp, WM_USER, 0, 104) == PLAYING)
					rubirc_printf(ph, "Winamp: playing");
				else
					rubirc_printf(ph, "Winamp: paused");
			}
		}
		else if (!stricmp("STOP", word[2]))
		{
			SendMessage(hwndWinamp, WM_COMMAND, 40047, 0);
			rubirc_printf(ph, "Winamp: stopped");
		}
		...
		else if (!stricmp("NEXT", word[2]))
		{
			...
			rubirc_printf(ph, "Winamp: next playlist entry");
		}
		else if (!stricmp("PREV", word[2]))
		{
			SendMessage(hwndWinamp, WM_COMMAND, 40044, 0);
			rubirc_printf(ph, "Winamp: previous playlist entry");
		}
		else if (!stricmp("START", word[2]))
		{
			SendMessage(hwndWinamp, WM_COMMAND, 40154, 0);
			rubirc_printf(ph, "Winamp: playlist start");
		}
		else if (!word_eol[2][0])
		{
			...
			if (!current_play)
			{
				rubirc_print (ph, "Winamp: Error getting song information.");
			...
			if (strchr(current_play, '-'))
			{
				...
				*++p = '\0';
				...
				if (*p != '\0')
					rubirc_commandf (ph, "me is now playing: %s", p);
				else
					rubirc_print (ph, "Winamp: No song information found.");
				g_free (current_play);
			}
			else
				rubirc_print(ph, "Winamp: Nothing being played.");
		}
		else
			rubirc_printf(ph, "Usage: /WINAMP [PAUSE|PLAY|STOP|NEXT|PREV|START]\n");
	}
	else
	{
		rubirc_print(ph, "Winamp not found.\n");
	}
	return RUBIRC_EAT_ALL;
}

int
rubirc_plugin_init(rubirc_plugin *plugin_handle,
					  char **plugin_name,
					  char **plugin_desc,
					  char **plugin_version,
					  char *arg)
{
	...
	*plugin_name = "Winamp";
	*plugin_desc = "Winamp plugin for RUBIRC";
	*plugin_version = "0.6";

	rubirc_hook_command (ph, "WINAMP", RUBIRC_PRI_NORM, winamp, "Usage: /WINAMP [PAUSE|PLAY|STOP|NEXT|PREV|START] - control Winamp or show what's currently playing", 0);
   	rubirc_command (ph, "MENU -ishare\\music.png ADD \"Window/Display Current Song (Winamp)\" \"WINAMP\"");

	rubirc_print (ph, "Winamp plugin loaded\n");
...
{
	rubirc_command (ph, "MENU DEL \"Window/Display Current Song (Winamp)\"");
	rubirc_print (ph, "Winamp plugin unloaded\n");
	return 1;
}
