/* dbus-client.c - RUBIRC command-line options for D-Bus
...
#define DBUS_REMOTE_PATH "/org/rubirc/Remote"
#define DBUS_REMOTE_INTERFACE "org.rubirc.plugin"
 ...
 */
{
	...
	/* Checks if RUBIRC is already running */
	...
	if (!ret)
	{
		...
		rubirc_running = FALSE;
	}
	else
	{
		GVariant *child = g_variant_get_child_value (ret, 0);
		rubirc_running = g_variant_get_boolean (child);
		g_variant_unref (ret);
	...
	if (!remote_object)
	{
		write_error("Failed to connect to RUBIRC", &error);
		g_object_unref (connection);
		exit (0);
	}
