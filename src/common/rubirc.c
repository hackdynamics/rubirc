...
static int in_rubirc_exit = FALSE;
int rubirc_is_quitting = FALSE;
/* command-line args */
...
static int
rubirc_lag_check (void)   /* this gets called every 30 seconds */
{
	lag_check ();
	return 1;
}
...
static int
rubirc_lag_check_update (void)   /* this gets called every 0.5 seconds */
{
	lagcheck_update ();
	return 1;
}
...
rubirc_reinit_timers (void)
{
	...
	{
		lag_check_update_tag = fe_timeout_add (500, rubirc_lag_check_update, NULL);
	}
	...
	if ((prefs.hex_net_ping_timeout != 0 || prefs.hex_gui_lagometer)
	    && lag_check_tag == 0)
	{
		lag_check_tag = fe_timeout_add_seconds (30, rubirc_lag_check, NULL);
	}
...
session_free (session *killsess)
{
	...
	if (!sess_list && !in_rubirc_exit)
		rubirc_exit ();						/* sess_list is empty, quit! */
...
{
#ifdef WIN32
	char rubirc_lang[13];	/* LC_ALL= plus 5 chars of hex_gui_lang and trailing \0 */

	strcpy (rubirc_lang, "LC_ALL=");

	if (0 <= prefs.hex_gui_lang && prefs.hex_gui_lang < LANGUAGES_LENGTH)
		strcat (rubirc_lang, languages[prefs.hex_gui_lang]);
	else
		strcat (rubirc_lang, "en");
