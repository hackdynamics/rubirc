...
#   gui_entry
#     This is rubirc_plugin pointer that is used to remove the script from
...
sub unload {
	...
	if( $pkg_info ) {	
		...
		return RUBIRC::EAT_ALL;
	} else {
		RUBIRC::print( qq{"$file" is not loaded.\n} );
		return RUBIRC::EAT_NONE;
	}
}
...
sub reload_all {
	my @dirs = RUBIRC::get_info( "configdir" );
...
sub evaluate {
	...
	RUBIRC::print $@ if $@; #print warnings

	local $Data::Dumper::Sortkeys = 1;
	local $Data::Dumper::Terse    = 1;

	if (@results > 1) {
		RUBIRC::print Dumper \@results;
	}
	elsif (ref $results[0] || !$results[0]) {
		RUBIRC::print Dumper $results[0];
	}
	else {
		RUBIRC::print $results[0];
	}

	return RUBIRC::EAT_RUBIRC;
};
...
sub find_external_pkg {
	...
	while( my @frame = caller( $level ) ) {
		return @frame if $frame[0] !~ /(?:^IRC$|^RUBIRC)/;
		$level++;
	}
	return;
}
...
sub find_pkg {
	...
	while( my ($package, $file, $line) = caller( $level ) ) {
		return $package if $package =~ /^RUBIRC::Script::/;
		$level++;
	}
