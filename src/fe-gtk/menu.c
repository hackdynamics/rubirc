...
{
	fe_open_url ("http://rubirc.readthedocs.org");
}
...
static struct mymenu mymenu[] = {
	{N_("RUBIRC"), 0, 0, M_NEWMENU, MENU_ID_RUBIRC, 0, 1},
						...
						GtkWidget **menu_widgets)
{
	...
	while (1)
	{
		...
		case M_NEWMENU:
			...
			if (!bar || mymenu[i].id != MENU_ID_RUBIRC)		
#endif
...
#ifdef HAVE_GTK_MAC
		/* We want RUBIRC to be the app menu, not including Quit or RUBIRC itself */
		if (bar && item && i <= CLOSE_OFFSET + 1 && mymenu[i].id != MENU_ID_RUBIRC)
		{
