...
namespace thememan
{
    public partial class HTM : Form
    {
        ...
        public HTM ()
        {
            ...
            {
                rubircdir = Path.Combine(portableDir, "config\\");

                if (!Directory.Exists(rubircdir))
                {
                    MessageBox.Show("RUBIRC installation not found!\nCheck your .\\config folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Environment.Exit(1);
                }
            }
            else
            {
                 ...
                 * Windows: %APPDATA%
                 * Unix: ~/.config
                 * Windows is case-insensitive so 'rubirc' should be fine for both
                 */
                rubircdir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "rubirc");

                if (!Directory.Exists(rubircdir))
                {
                    if (RunningOnWindows())
                    {
                        MessageBox.Show("RUBIRC installation not found!\nCheck your %APPDATA%\\RUBIRC folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);    
                    }
                    else
                    {
                        MessageBox.Show("RUBIRC installation not found!\nCheck your ~/.config/rubirc folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
            ...
            themedir = Path.Combine(rubircdir, "themes");
            ListThemes();
        ...
        {
            DialogResult result = MessageBox.Show("RUBIRC must be closed and this will overwrite your current theme!\n\nDo you wish to continue?", "Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (result == DialogResult.OK)
            {
                File.Copy(Path.Combine(themedir, themelist.SelectedItem.ToString(), "colors.conf"), Path.Combine(rubircdir, "colors.conf"), true);
                if (File.Exists(Path.Combine(themedir, themelist.SelectedItem.ToString(), "pevents.conf")))
                {
                    File.Copy(Path.Combine(themedir, themelist.SelectedItem.ToString(), "pevents.conf"), Path.Combine(rubircdir, "pevents.conf"), true);
                }
                else if (File.Exists(Path.Combine(rubircdir, "pevents.conf")))
                {
                    File.Delete(Path.Combine(rubircdir, "pevents.conf"));
                }
            }
        }
        ...
        private void importbutton_Click_1(object sender, EventArgs e)
        {
            ...
            importDialog.Filter = "RUBIRC Theme Files|*.hct";
