...
notify_save (void)
{
	...
	fh = rubirc_open_file ("notify.conf", O_TRUNC | O_WRONLY | O_CREAT, 0600, XOF_DOMODE);
	if (fh != -1)
	{
		while (list)
		{
...
notify_load (void)
{
	...
	fh = rubirc_open_file ("notify.conf", O_RDONLY, 0, 0);
	if (fh != -1)
	{
...
notify_markonline (server *serv, char *word[], const message_tags_data *tags_data)
{
	...
	while (list)
	{
		...
		while (*word[i])
		{
			...
			if (i > PDIWORDS - 5)
			{
				/*fprintf (stderr, _("*** RUBIRC WARNING: notify list too large.\n"));*/
				break;
			}
		}
