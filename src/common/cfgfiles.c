...
get_xdir (void)
{
	...
	{
#ifndef WIN32
		xdir = g_build_filename (g_get_user_config_dir (), RUBIRC_DIR, NULL);
#else
		...
		else
		{
			...
			xdir = g_build_filename (roaming_path, "RUBIRC", NULL);
...
default_file (void)
{
	...
	if (!dfile)
	{
		dfile = g_build_filename (get_xdir (), "rubirc.conf", NULL);
	}
	return dfile;
}
...
const struct prefs vars[] =
{
	...
	{"net_ping_timeout", P_OFFINT (hex_net_ping_timeout), TYPE_INT, rubirc_reinit_timers},
...
load_default_config(void)
{
	...
	memset (&prefs, 0, sizeof (struct rubircprefs));
...
make_config_dirs (void)
{
	...
	buf = g_build_filename (get_xdir (), RUBIRC_SOUND_DIR, NULL);
	if (g_mkdir (buf, 0700) != 0)
	{
		g_free (buf);
		return -1;
...
int
rubirc_open_file (const char *file, int flags, int mode, int xof_flags)
{
	char *buf;
	int fd;
...
FILE *
rubirc_fopen_file (const char *file, const char *mode, int xof_flags)
{
	char *buf;
	FILE *fh;
